...
try {
    ...
    # Preserve tracked executable bits, including on Windows with core.filemode=false.
    $trackedModes = [System.Collections.Generic.Dictionary[string, string]]::new([StringComparer]::Ordinal)
    $index = Invoke-Git -Arguments @('-C', $sourceRoot, 'ls-files', '--stage', '-z', '--', 'AzureDevOps/Projects/')
    foreach ($entry in $index.Split([char]0, [StringSplitOptions]::RemoveEmptyEntries)) {
        if ($entry -notmatch '(?s)^(\d+) [0-9a-f]+ (\d)\t(.+)$') { throw 'Unrecognized Git index entry.' }
        if ($Matches[2] -ne '0') { throw "Resolve merge conflicts before replication: $($Matches[3])" }
        $trackedModes[$Matches[3]] = $Matches[1]
    }
    foreach ($sourcePath in ($paths.Split([char]0, [StringSplitOptions]::RemoveEmptyEntries) | Sort-Object -Unique -CaseSensitive)) {
        ...
        $mode = if ($trackedModes.ContainsKey($sourcePath)) { $trackedModes[$sourcePath] } else { '100644' }
        if ($mode -notin @('100644', '100755')) { throw "Unsupported Git file mode $mode for $sourcePath" }
