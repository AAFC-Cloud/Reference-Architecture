...
try {
    ...
    # Local file resources must preserve binary bytes as well as UTF-8 text.
    $binaryPath = Join-Path $fixtureRoot "$repoA/binary.dat"
    [System.IO.File]::WriteAllBytes($binaryPath, [byte[]]@(0, 1, 2, 255, 254))
    $binaryCatalog = Read-Catalog
    if (-not $binaryCatalog.files.ContainsKey('Project A/Infrastructure/binary.dat')) {
        throw 'Binary files must be included in the source inventory.'
    }
    ...
    Write-Output 'Passed discovery, exclusions, deletion, binary support, Terraform mapping and Git synchronization checks.'
} finally {
